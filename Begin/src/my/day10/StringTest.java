...
public class StringTest {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<str.length(); i++){
			System.out.println(str.charAt(i));//글자한개만 읽어온다
		}
		...
		char[] charArr = str.toCharArray();//해당배열을 문자열(char)타입으로 가져온다
		...
		System.out.println("\n===4.String 문자열.substring(int beginIndex)");
		...
		food = "골뱅이.치킨.맥주.과일.닭발.소주.피자";
	//	foodArr = food.split("."); 이거안돼 .한개는 글자 한개로 식별
		foodArr = food.split("\\.");
		for(int i=0; i<foodArr.length; i++) {
			System.out.println(foodArr[i]);
		}
	    ...
	    */
		...
		// String String.join(구분자, 문자열 배열) 
		// ==> 문자열배열 구분자로 합쳐서 한개의 문자열로 반환.
		
		 System.out.println("==== 7. String String.join(구분자, 문자열 배열) ==== ");
		 ...
		 for(int i=0; i<fileNameArr2.length; i++) {
			 if(fileNameArr2[i].startsWith("안녕")) {//안녕으로 시작하는 파일명찾기
